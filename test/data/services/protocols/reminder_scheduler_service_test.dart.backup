...
void main() {
  ...
  group('scheduleProtocolReminders()', () {
    ...
    test(
      ...
      // Verify reminder IDs match pattern
      final capturedReminders = verify(() =>
              mockNotificationService.scheduleReminder(captureAny()))
          .captured
          .cast<Reminder>();
      expect(capturedReminders[0].id, 'pet-123_vaccination_vacc-456_1');
      expect(capturedReminders[1].id, 'pet-123_vaccination_vacc-456_7');
    });
  ...
  group('cancelReminders()', () {
    ...
    test(
      ...
      // Assert - should attempt to cancel [0, 1, 7, 14, 30]
      expect(count, 5);
      verify(() => mockNotificationService
          .cancelReminder('pet-123_vaccination_vacc-456_0')).called(1);
      verify(() => mockNotificationService
          .cancelReminder('pet-123_vaccination_vacc-456_1')).called(1);
      verify(() => mockNotificationService
          .cancelReminder('pet-123_vaccination_vacc-456_7')).called(1);
      verify(() => mockNotificationService
          .cancelReminder('pet-123_vaccination_vacc-456_14')).called(1);
      verify(() => mockNotificationService
          .cancelReminder('pet-123_vaccination_vacc-456_30')).called(1);
    });
  ...
  group('calculateReminderDates()', () {
    ...
    test('should calculate reminder date for large offset (90 days)', () {
      ...
      // Assert - 90 days before Dec 15 is Sep 16
      expect(result.length, 1);
      expect(result[90]!.month, 9);
      expect(result[90]!.day, 16);
  ...
  group('Helper Methods & Edge Cases', () {
    ...
    test('_mapEventTypeToReminderType() should map event types correctly', () {
      // We can't directly test private methods, but we can verify through public API
      // This is implicitly tested in scheduleProtocolReminders tests
      ...
      // Act & Assert - verify ReminderType for each event type
      service
        ...
        final capturedReminder = verify(() =>
            ...
            .captured
            .first as Reminder;
        expect(capturedReminder.type, ReminderType.medication);
      });
      ...
      service
        ...
        final capturedReminder = verify(() =>
            ...
            .captured
            .first as Reminder;
        expect(capturedReminder.type, ReminderType.appointment);
      });
    });
    ...
    test('Integration: End-to-end scheduling flow', () async {
      ...
      final capturedReminders = verify(() =>
          ...
          .captured
          .cast<Reminder>();
      expect(capturedReminders[0].petId, 'pet-123');
      expect(capturedReminders[0].type, ReminderType.medication);
      expect(capturedReminders[0].title, contains('Rabies'));
      expect(capturedReminders[0].frequency, ReminderFrequency.once);
      expect(capturedReminders[0].linkedEntityId, contains('vacc_0_Rabies'));
    });
  });
}
