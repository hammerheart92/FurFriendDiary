...
/// - All PDF report files (health reports, vet summaries)
...
class DataDeletionService {
  ...
  Future<bool> deleteAllUserData() async {
    try {
      ...
      _logger.i('Step 1/6: Deleting Hive boxes...');
      await _deleteAllHiveBoxes();
      _logger.i('✅ Hive boxes deleted');

      // Step 2: Delete all photo files
      _logger.i('Step 2/6: Deleting photo files...');
      await _deleteAllPhotoFiles();
      _logger.i('✅ Photo files deleted');

      // Step 3: Delete all PDF report files
      _logger.i('Step 3/6: Deleting PDF files...');
      await _deleteAllPdfFiles();
      _logger.i('✅ PDF files deleted');

      // Step 4: Delete encryption keys (primary AND fallback)
      _logger.i('Step 4/6: Deleting encryption keys...');
      await _deleteEncryptionKeys();
      _logger.i('✅ Encryption keys deleted');

      // Step 5: Clear SharedPreferences
      _logger.i('Step 5/6: Clearing SharedPreferences...');
      await _clearSharedPreferences();
      _logger.i('✅ SharedPreferences cleared');

      // Step 6: Clear cache directories
      _logger.i('Step 6/6: Clearing cache directories...');
  ...
  /// Delete all PDF report files
  ///
  /// PDFs are stored in: {ApplicationDocumentsDirectory}/reports/
  /// This includes health reports and vet summaries generated by PDFExportService
  Future<void> _deleteAllPdfFiles() async {
    try {
      final appDir = await getApplicationDocumentsDirectory();
      final reportsDir = Directory('${appDir.path}/reports');

      if (await reportsDir.exists()) {
        _logger.d('Deleting reports directory: ${reportsDir.path}');
        await reportsDir.delete(recursive: true);
        _logger.i('Deleted reports directory: ${reportsDir.path}');
      } else {
        _logger.d('Reports directory does not exist, skipping');
      }
    } catch (e) {
      _logger.e('Error deleting PDF files: $e');
      rethrow;
    }
  }
